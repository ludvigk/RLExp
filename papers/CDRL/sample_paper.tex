...
Let \(\bm{Z}\) be a random variable defined over the support \(S \subseteq \R^n\)
...
\(T \in \R^n\), if we let \(\bm{Z}^\prime = v(\bm{Z})\), then the probability
function \(q(\rvz^\prime)\) of \(\bm{Z}^\prime\) is given by
\begin{align}
  ...
  &= q(v^{-1}(\rvz^\prime)) \left\lvert \det \left( \frac{\partial}{\partial
  \rvz^\prime} v(\rvz^\prime) \right) \right\rvert^{-1}
...
\(Z^\pi\), with a continous estimate.

\begin{equation}
  \mathcal{L}_{x,a}(\theta) = \KL{\hat{\T}
  Z_{\tilde{\theta}}(x,a)}{Z_{\theta}(x,a)}
\end{equation}
