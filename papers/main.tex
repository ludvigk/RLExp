...
Now the stochastic processes is represented by a neural network \(q_\phi\) and a prior \(p\). In a similar
...
\begin{equation}
    \mathcal{L} = \frac{1}{\lvert \D_s \rvert}\sum_{i=1}^{\lvert \D_s \rvert} \rho_i + \left( \mu_i - q_i \right)^2
    \exp (-\rho_i) + \lambda \KL[q \Vert p].
\end{equation}
...
has hyperparameters related to the functional KL-divergence. These are shown in
...
gives no incentive to maintain any uncertainty. Figure~\ref{fig:mean_std} shows
