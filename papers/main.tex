...
\title{Scalable and Parameter-Free Optimal Regret Reinforcement Learning}
...
\(J^\pi \defeq \E_\pi \left[ \sum_{t=1}^\infty \gamma^t r_t \right]\). An efficient
...
following that policy, and the expected reward for an optimal policy.
...
for this problem, but perhaps the simplest one is called Thompson sampling.
...
highest \(Q\)-value in any state. We can't find \(Q\) exactly, but with a good
approximation this policy, \(\pi(a \vert s) = \arg\max_a Q(s,a)\), should
hopefully be close to optimal. In deep RL, we model \(Q\) with a
...
\begin{itemize}
    \item Optimal regret \textcolor{red}{TODO: define optimal regret}
...
\(q_\phi\) comes from the true Q-value function.} \textcolor{green}{This should probably go under method}
