...
\citet{osband_why_2017} proove that in finite horizon MDPs posterior sampling for
...
\(N\) samples from Q. This lets us add the KL-term from FVBNN to the loss function that
encourages stochasticity.
The maximization target becomes:
\begin{equation}
    \log p(\D_\omega^N \vert f) - \KL \left[ q(\f^{\D_\omega^N}, \f^\mathbf{M}) \Vert p(\f^{\D_\omega^N}, \f^\mathbf{M}) \right],
\end{equation}
...
in environments without the advantage of prior knowledge.
We hypothesize that the added KL-term will push the algorithm to keep exploring unexplored
state-action pairs more than NoisyNet, thus 
