...
function (l::NoisyDense)(x, num_samples::Union{Int, Nothing}=nothing; rng::Union{AbstractRNG, Nothing}=nothing)
    ...
    if num_samples === nothing
        ...
        wϵ_1 = Zygote.@ignore randn!(rng, similar(μ, size(μ, 1), 1, 1))
        wϵ_2 = Zygote.@ignore randn!(rng, similar(μ, 1, size(μ, 2), 1))
        wϵ_3 = Zygote.@ignore randn!(rng, similar(μ, 1, 1, num_samples))
        wϵ = Zygote.@ignore wϵ_1 .* wϵ_2 .* wϵ_2
        bϵ_1 = Zygote.@ignore randn!(rng, similar(μ, size(μ, 1), 1, 1))
